...
## PRD 작성 원칙 (Figma 수준 정밀도)

PRD는 Figma 디자인 명세와 동일한 수준의 정밀도로 작성한다 — Figma 디자인이 픽셀 단위까지 모호함 없이 확정되어 있어 그대로 구현하면 되는 것처럼, PRD도 **명시된 모든 기능이 예외 없이, 정확히 구현되도록** 작성한다.

- 각 기능(Feature)마다 입력값, 상태 전이, 데이터 흐름, UI 동작, 엣지 케이스(빈 값/에러/권한 없음 등)까지 빠짐없이 명시한다.
- "적절히 처리한다", "필요시 구현한다"처럼 해석의 여지를 남기는 표현을 금지한다 — 개발자가 PRD만 보고 다르게 구현할 수 있는 여지가 있다면 그 항목은 미완성으로 간주한다.
- REQUIREMENT.md에 없는 세부사항을 임의로 추측해 채우지 않는다. 불명확한 부분은 PRD의 `## Open Items (미결 사항)`에 명시하고, 4단계(사용자 승인) 전에 반드시 확인받는다.
- 기능 명세·메뉴 구조·페이지별 상세 기능 간 불일치나 누락이 있으면 "구현 가능한 상태"가 아니므로 승인 요청 전 반드시 해소한다.
